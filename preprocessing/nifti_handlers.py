...
def convert_DICOM_to_NIFTI(root_dir, dcm2niix_bin):
    ...
    for root, dirs, files in os.walk(root_dir):
        if len(files) > 0 and 'MR' in root:
            ...
            nifti_file_name = '+'.join(sub_directory_names)

            system_command = dcm2niix_bin + ' -6 -z y -a y -b n -d 0 -e n -f ' + nifti_file_name + ' -g n -i n -l y -w 0 -o ' + temp_dir + ' ' + root
            print("[DCM2NIIX] ", system_command)
            # print(*natsorted(os.listdir(root)), sep="\n")
            ...
            if len(nifti_out_files) > 0:
                print(f"[DEBUG] moving nifti file to --> {os.path.join(patient_out_folder, nifti_file_name + '.nii.gz')}")
        ...
        for root, dirs, files in os.walk(root_dir):
            for i_file in natsorted(files):
                if '.nii.gz' in i_file:
                    ...
                    file_location = os.path.join(i_file)
